...
export const generateDistributorReport = async (
  ...
  try {
    ...
    autoTable(doc, {
      ...
      body: [
        ['Produit :', productName],
        ['Prix Unitaire :', `${unitPrice.toLocaleString()} MRU`],
      ],
    ...
    autoTable(doc, {
      ...
      body: [
        ['Chiffre d\'Affaires Brut', grossRevenue.toLocaleString()],
        ['Frais de Livraison', `- ${deliveryCosts.toLocaleString()}`],
        ['TOTAL NET', { content: `${netRevenue.toLocaleString()} MRU`, styles: { fontStyle: 'bold' } }],
      ],
      headStyles: { 
        textColor: 0, 
        fontStyle: 'bold',
        lineWidth: { bottom: 0.1 }, // Correction ici : On définit l'épaisseur de la ligne
        lineColor: 200 
      },
      margin: { left: 14 },
      didDrawPage: (data) => { finalY = data.cursor ? data.cursor.y + 12 : finalY + 20; }
...
// On force le type pour fontStyle
const boldStyle = 'bold' as const;

// --- CALCULS ---
const totalVendu = finishedOrders.reduce((sum, o) => sum + o.quantity, 0);
const enCoursOrders = orders.filter(o => !['TERMINEE', 'PAYEE', 'ANNULEE'].includes(o.status));
const totalEnCours = enCoursOrders.reduce((sum, o) => sum + o.quantity, 0);
const ecart = cycle.initialStock - totalVendu - totalEnCours - cycle.remainingStock;
...
autoTable(doc, {
  ...
  body: [
    ...
    [
      'Unités Vendues (Livrées)', 
      { content: String(totalVendu), styles: { textColor: green, fontStyle: boldStyle } }
    ],
    [
      'Unités en Cours (Commandes)', 
      { content: String(totalEnCours), styles: { textColor: orange } }
    ],
    [
      'Stock Physique Restant', 
      { content: String(cycle.remainingStock), styles: { fontStyle: boldStyle } }
    ],
    // Ligne d'écart (n'apparaît que si nécessaire)
    ...(ecart !== 0 ? [[
      'Écart / Manquant', 
      { content: String(ecart), styles: { textColor: red, fontStyle: boldStyle } }
    ]] : []),
  ],
  headStyles: { 
    textColor: 0, 
  ...
  didDrawPage: (data) => { if (data.cursor) finalY = data.cursor.y + 15; }
    ...
    doc.setFontSize(12);
    doc.text('Détail des Ventes', 14, finalY);

    autoTable(doc, {
      startY: finalY + 2,
      theme: 'striped',
      head: [['Client', 'Zone', 'Qté', 'Total (MRU)', 'Statut']],
      headStyles: { fillColor: [41, 128, 185], textColor: 255 }, // Header bleu pour la liste longue
      body: orders.map(o => [
        o.clientName,
        o.deliveryZone === 'Wilaya' ? o.address : 'Nouakchott',
        o.quantity,
